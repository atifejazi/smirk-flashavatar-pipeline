...
if __name__ == '__main__':
    ...
    output_file = os.path.join(args.out_path, f"{video_name}.mp4")
    print(f"Saving video to: {output_file}")
    #cap_out = cv2.VideoWriter(output_file, cv2.VideoWriter_fourcc(*'mp4v'), video_fps, (out_width, out_height))
    ...
    while True:
        ...
        # cop l v 3 seq
        # Grab the vertices for this frame
        current_verts = flame_output['vertices'].detach().cpu().numpy()
        
        # Save the first frame's geometry as the Template Mesh
        if template_mesh is None:
            template_mesh = current_verts[0]
            
        # Add the current frame to our sequence
        all_vertices.append(current_verts[0])
        ...
        # print()
        frame_data = {
            'beta': outputs['shape_params'].detach().cpu().numpy(),
            'psi': outputs['expression_params'].detach().cpu().numpy(),
            'theta': outputs['pose_params'].detach().cpu().numpy()
        }
        # to a file
        frame_idx = int(cap.get(cv2.CAP_PROP_POS_FRAMES))
        np.save(f"{args.out_path}/frame_{frame_idx:04d}.npy", frame_data)
    ...
    if len(all_vertices) > 0:
        subject_id = video_name[:4]
        mesh_sequence = np.stack(all_vertices)
        
        np.save(os.path.join(args.out_path, f"{video_name}_mesh_sequence.npy"), mesh_sequence)
        np.save(os.path.join(args.out_path, f"{subject_id}_template_mesh.npy"), template_mesh)
        print(f"Successfully saved {video_name}_mesh_sequence.npy and template mesh!")
