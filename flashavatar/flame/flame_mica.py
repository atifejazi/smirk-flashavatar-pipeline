...
class FLAME_mica(nn.Module):
    ...
    def forward_geo(self, shape_params, trans_params=None, rot_params=None, neck_pose_params=None, jaw_pose_params=None, eye_pose_params=None, expression_params=None, eyelid_params=None):
        ...
        def _ensure_rot6d(pose_params, joints):
            if pose_params is None:
                return None
            target_dim = 6 * joints
            if pose_params.shape[1] == target_dim:
                return pose_params
            if pose_params.shape[1] == 3 * joints:
                pose_aa = pose_params.view(batch_size, joints, 3)
                pose_mat = axis_angle_to_matrix(pose_aa)
                return matrix_to_rotation_6d(pose_mat).view(batch_size, target_dim)
            if pose_params.shape[1] < target_dim:
                pad = torch.zeros(batch_size, target_dim - pose_params.shape[1], device=pose_params.device, dtype=pose_params.dtype)
                return torch.cat([pose_params, pad], dim=1)
            return pose_params[:, :target_dim]
        ...
        rot_params      = _ensure_rot6d(rot_params,      joints=1)
        neck_pose_params= _ensure_rot6d(neck_pose_params, joints=1)
        jaw_pose_params = _ensure_rot6d(jaw_pose_params,  joints=1)
        eye_pose_params = _ensure_rot6d(eye_pose_params,  joints=2)
