...
class Deform_Model(nn.Module):
    ...
    def decode(self, codedict):
        ...
        global_rot = codedict.get('global_rot', None)
        if global_rot is not None:
            global_rot = global_rot.detach()
        batch_size = shape_code.shape[0]
        condition = torch.cat((expr_code, jaw_pose, eyes_pose, eyelids), dim=1)
        expected_cond_dim = self.deformNet.input_dim - self.pts_embedder.dim_embeded
        if condition.shape[1] != expected_cond_dim:
            # Pad or trim to match the network's expected condition size.
            if condition.shape[1] < expected_cond_dim:
                pad = expected_cond_dim - condition.shape[1]
                condition = F.pad(condition, (0, pad))
            else:
                condition = condition[:, :expected_cond_dim]
